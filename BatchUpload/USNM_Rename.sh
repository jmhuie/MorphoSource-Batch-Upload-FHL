...
do
    ...
    if [[ $Source == "USNM" ]] #tests you're working with stuff from the correct museum
    then
        Museum="Usnm" #sets museum with correct cases for next script
        Collection="F" #sets to F for fish
        echo $Museum #reads them out
        echo $Collection
    else
        echo "Not USNM" # tells you if wrong museum
    fi
