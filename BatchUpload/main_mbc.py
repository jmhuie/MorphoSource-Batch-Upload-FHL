...
#%% Scan metadata files########################################################
#Notes files
if uc.NOTES_METADATA_FOLDER is not None: #works for either batch or not
    #run the extract settings script, modified temporarily
    NOTESInputPath = uc.INPUT_PATH + '/' + uc.NOTES_METADATA_FOLDER
    NOTESdf = nts.ntsmeta_from_raw_files(NOTESInputPath,uc.NAME_SCAN)
...
Worksheet = nts.fill_notes(Worksheet,NOTESdf)
