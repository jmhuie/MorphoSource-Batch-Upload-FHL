...
do
    ...
    if [[ $Source == "ROM" ]] #tests you're working with stuff from the correct museum
    then
        Museum="Rom" #sets museum with correct cases for next script
        Collection="F" #sets to F for fish
        echo $Museum #reads them out
        echo $Collection
    else
        echo "Not ROM" # tells you if wrong museum
    fi
