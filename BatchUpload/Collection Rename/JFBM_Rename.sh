...
do
    ...
    if [[ $Source == "JFBM" ]] #tests you're working with stuff from the correct museum
    then
        Museum="Jfbm" #sets museum with correct cases for next script
        Collection="F" #sets to F for fish
        echo $Museum #reads them out
        echo $Collection
    else
        echo "Not JFBM" # tells you if wrong museum
    fi
